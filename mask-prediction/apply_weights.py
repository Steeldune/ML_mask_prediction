...
def add_circles(ctrl_points, diameter):
    ret_img = np.ones((1024, 1024), dtype=int)*255
    for pnt in ctrl_points:
        cv2.circle(ret_img, pnt, diameter//2, 0, thickness=-1)
    return ret_img
...
def add_gauss(image, ctrl_points, diameter):
    ...
    for pnt in ctrl_points:
        ...
        gauss_map = np.exp((dist_map*-1)/diameter)*255
        out_image = out_image + gauss_map
